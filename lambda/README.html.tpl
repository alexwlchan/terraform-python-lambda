<html>
  ...
  <body>
    <main>
      <figure>
      <svg viewBox="0 0 450 295" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <style>
            line {
              stroke: #222;
        ...
        <path d="M 24 52 L 24 89" stroke="black"
        stroke-width="1.75" fill="none"  marker-end="url(#arrowhead)" />

        <path d="M 24 159 L 24 241" stroke="black"
        stroke-width="1.75" fill="none"  marker-end="url(#arrowhead)" />
      ...
      <p>
        To send a single input message:
      </p>

      <pre><code>env AWS_PROFILE=${account_name}-dev aws sns publish \
  --topic-arn=${topic_arn} \
  --message='{"message": "hello world"}'</code></pre>

      <p>
        To send lots of input messages:
      </p>

      <pre><code>bulk_sns_publish \
  --topic-arn=${topic_arn} \
  input.txt</code></pre>

      <p>
        To get the function’s output:
      </p>

      <pre><code>download_sqs_messages ${queue_url}</code></pre>
    </main>
  </body>
</html>
